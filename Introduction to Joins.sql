...
select d.department_name,count(e.employee_id)as employee_count from hr.departments d inner join hr.employees e
on d.department_id=e.department_id where e.job_id = 'SA_REP' GROUP BY d.department_id, d.department_name;

--H14. Show employee name, department_name, and salary rank within that department (RANK() OVER (PARTITION BY department_id 
--ORDER BY salary DESC)).
--Hint: Join; in SELECT add RANK() OVER (PARTITION BY e.department_id ORDER BY e.salary DESC) AS rank_in_dept.

select e.first_name,e.last_name,d.department_name,e.salary, rank() over (PARTITION BY e.department_id ORDER BY e.salary DESC) AS rank_in_dept 
from hr.employees e inner join hr.departments d on e.department_id=d.department_id;

--H15. List department_name and total number of employees, including departments with 0 employees (use LEFT JOIN 
...
select d.department_name,count(e.employee_id) as total_employee from hr.departments d left join hr.employees e ON d.department_id = e.department_id 
GROUP BY d.department_id,d.department_name;
...
select d.department_name,AVG(MONTHS_BETWEEN(SYSDATE, e.hire_date)/12) from hr.employees e inner join hr.departments d on e.department_id=
d.department_id group by d.department_id,d.department_name;

--H18. Show employee_id, first_name, last_name, department_name, and number of employees in that department 
--(same count repeated per employee in dept).
--Hint: Join and join to (SELECT department_id, COUNT() cnt FROM hr.employees GROUP BY department_id) sub 
--ON e.department_id = sub.department_id; select e., d.department_name, sub.cnt.
...
SELECT d.department_name,COUNT(*) AS employee_count FROM hr.departments d INNER JOIN hr.employees e ON d.department_id = e.department_id
WHERE e.salary > 5000 GROUP BY d.department_id,d.department_name HAVING COUNT(*) > 2;

--H20. Show first_name, last_name, department_name, and salary as percentage of department total (salary * 100.0 / SUM(salary) OVER 
--(PARTITION BY e.department_id)).
--Hint: Join; add salary * 100.0 / SUM(e.salary) OVER (PARTITION BY e.department_id) AS pct_of_dept_sal.
SELECT e.first_name, e.last_name, d.department_name, e.salary,e.salary * 100.0/ SUM(e.salary) OVER (
        PARTITION BY e.department_id
        ) AS pct_of_dept_sal
FROM hr.employees e INNER JOIN hr.departments d ON e.department_id = d.department_id;
